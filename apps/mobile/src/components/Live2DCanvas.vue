  ...
  try {
      ...
      if (motionManager?.definitions) {
        ...
        for (const group in definitions) {
          ...
          if (groupMotions && groupMotions.length > 0) {
            ...
            // 提取组内每个动作的详细信息
            groupMotions.forEach((motion: { Name?: string; name?: string; File?: string }, index: number) => {
              // 尝试获取动作名称，如果没有则使用文件名或索引
              let motionName = motion.Name || motion.name
              if (!motionName && motion.File) {
                // 从文件名提取动作名（去掉路径和扩展名）
                motionName = motion.File.split('/').pop()?.replace(/\.motion3?\.json$/i, '') || `${group}_${index}`
              }
              if (!motionName) {
    ...
    try {
      ...
      if (expressionManager?.definitions) {
        const expressions = expressionManager.definitions.map((def: { name?: string; Name?: string }) =>
    ...
    console.warn = function(...args: unknown[]) {
      ...
      if (message.includes('non-power-of-2') || message.includes('RENDER WARNING') || message.includes('not renderable')) {
...
<template>
  <div class="live2d-container">
    ...
    <!-- 渲染警告提示 -->
    <div v-if="renderWarning" class="render-warning" @click="renderWarning = false">
      <div class="warning-content">
        ...
        <div class="warning-text">
          该模型可能在移动端无法正常显示。
          <br><br>
          <strong>原因：</strong>模型纹理尺寸不是 2 的幂次方（如 256、512、1024）
          <br><br>
          <strong>解决方案：</strong>
          <br>1. 使用图像编辑软件将纹理调整为 2 的幂次方尺寸
          <br>2. 使用 Cubism Editor 重新导出模型
          <br>3. 更换其他兼容的模型
        </div>
